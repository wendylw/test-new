...
import { getStoreDisplaySubTitle } from '../common/selectors';
...
const { API_REQUEST_STATUS, ADDRESS_RANGE, DELIVERY_METHOD } = Constants;
...
export const getStoreList = createSelector(
  ...
  (selectedStoreId, currentDate, shippingType, storeList, addressCoords, deliveryRadius, businessUTCOffset) => {
    ...
    const getStoreDeliveryDistance = store => {
      if (shippingType !== DELIVERY_METHOD.DELIVERY || _isEmpty(addressCoords)) {
        return NaN;
      }
    ...
    return storeList
      ...
      .map(store => {
        ...
        return {
          ...
          outOfRange,
          displayOpeningTime,
          displayDistance: shippingType === DELIVERY_METHOD.DELIVERY && !_isEmpty(distance),
        };
      })
